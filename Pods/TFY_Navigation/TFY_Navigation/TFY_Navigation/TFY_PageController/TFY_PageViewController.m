...
typedef void(^TFY_ContentScollBlock)(BOOL scrollEnabled);

@interface TFY_PageContentView : UIView

@property (nonatomic, strong) TFY_ContentScollBlock scrollBlock;

@end

@implementation TFY_PageContentView

//兼容和子view滚动冲突问题
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    UIView *view =  [super hitTest:point withEvent:event];
    BOOL pageViewScrollEnabled = !view.tfy_letMeScrollFirst;
    if (self.scrollBlock) {
...
@interface TFY_PageViewController ()<UIPageViewControllerDelegate, UIPageViewControllerDataSource,UIScrollViewDelegate,TFY_PageTitleViewDataSrouce,TFY_PageTitleViewDelegate>
//所有的子视图，都加载在contentView上
@property (nonatomic, strong) TFY_PageContentView *contentView;
//标题
@property (nonatomic, strong) TFY_PageBasicTitleView *titleView;
//分页控制器
...
@implementation TFY_PageViewController
...
- (void)initUIWithConfig:(TFY_PageControllerConfig *)config {
    
    //保存配置
    self.config = config;
    
    //创建contentview
    self.contentView = [[TFY_PageContentView alloc] init];
    ...
    //创建标题
    self.titleView = [[TFY_PageBasicTitleView alloc] initWithConfig:config];
    if (config.titleViewStyle == TFY_PageTitleViewStyleSegmented) {
        self.titleView = [[TFY_PageSegmentedTitleView alloc] initWithConfig:config];
    }
    ...
    //兼容全屏返回手势识别
    self.scrollView.tfy_otherGestureRecognizerBlock = ^BOOL(UIGestureRecognizer * _Nonnull otherGestureRecognizer) {
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray<UIViewController *> *)previousViewControllers transitionCompleted:(BOOL)completed {
    ...
    _selectedIndex = [self.allTitleArr indexOfObject:vc.tfy_title];
    //标题居中
...
//titleview数据源方法
- (TFY_PageTitleCell *)pageTitleViewCellForItemAtIndex:(NSInteger)index {
...
- (BOOL)switchToViewControllerAdIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    //如果正在加载中 返回
    if (self.pageVCAnimating && self.config.titleViewStyle == TFY_PageTitleViewStyleBasic) {return NO;}
...
- (TFY_PageTitleCell *)dequeueReusableTitleViewCellWithIdentifier:(NSString *)identifier forIndex:(NSInteger)index {
...
- (UIViewController *)viewControllerForIndex:(NSInteger)index {
    ...
    //当前标题
    NSString *currentTitle = currentVC.tfy_title;
    //目标切换位置标题
    ...
    for (UIViewController *vc in self.shownVCArr) {
        if ([vc.tfy_title isEqualToString:targetTitle]) {
            return vc;
        }
    }
    ...
    //设置扩展id，用户定位对应视图控制器
    vc.tfy_title = [self titleForIndex:index];
    //设置试图控制器标题
