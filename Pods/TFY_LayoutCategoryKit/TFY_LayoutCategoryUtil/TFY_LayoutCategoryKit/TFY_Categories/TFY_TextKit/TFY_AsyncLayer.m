...
@interface TFY_Sentinel : NSObject
...
@implementation TFY_AsyncLayer{
    TFY_Sentinel *_sentinel;
}
...
- (void)configureLayer {
    _sentinel = [[TFY_Sentinel alloc]init];
    _displaysAsynchronously = YES;
    self.contentsScale = tfy_text_screen_scale();
    self.opaque = YES;
}
...
- (void)displayAsync:(BOOL)asynchronously {
    __strong id<AsyncLayerDelegate> delegate = _asyncDelegate;
    TFY_AsyncLayerDisplayTask *task = [delegate newAsyncDisplayTask];
    if (task.willDisplay) {
    ...
    if (asynchronously) {
        ...
        TFY_Sentinel *sentinel = _sentinel;
...
@implementation TFY_AsyncLayerDisplayTask
@end
