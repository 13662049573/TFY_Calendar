...
@interface TFY_TagCollectionView ()
...
@implementation TFY_TagCollectionView
...
- (void)layoutTagViews {
    ...
    if (_scrollDirection == TagCollectionScrollDirectionVertical) {
    ...
    for (NSUInteger currentLine = 0; currentLine < numberOfLines; currentLine++) {
        ...
        switch (_alignment) {
            ...
            case TagCollectionAlignmentFillByExpandingSpace:
                ...
                break;
            case TagCollectionAlignmentFillByExpandingWidth:
            case TagCollectionAlignmentFillByExpandingWidthExceptLastLine:
                currentLineXOffset = _contentInset.left;
                currentLineAdditionWidth = (maxLineWidth - currentLineWidth) / (CGFloat)currentLineTagsCount;
                currentLineWidth = maxLineWidth;
                
                if (_alignment == TagCollectionAlignmentFillByExpandingWidthExceptLastLine &&
                    currentLine == numberOfLines - 1 &&
                    numberOfLines != 1) {
                    // Reset last line width for TagCollectionAlignmentFillByExpandingWidthExceptLastLine
        ...
        [eachLineTagIndexs[currentLine] enumerateObjectsUsingBlock:^(NSNumber * _Nonnull tagIndexNumber, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (self.scrollDirection == TagCollectionScrollDirectionVertical && tagSize.width > maxLineWidth) {
...
- (NSUInteger)actualNumberOfLines {
    if (_scrollDirection == TagCollectionScrollDirectionHorizontal) {
