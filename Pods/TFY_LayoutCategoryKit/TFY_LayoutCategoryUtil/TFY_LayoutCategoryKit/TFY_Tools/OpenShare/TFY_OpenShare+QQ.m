...
@implementation TFY_OpenShare (QQ)
...
+(void)QQAuth:(NSString*)scope Success:(authSuccess)success Fail:(authFail)fail{
    if ([self beginAuth:schema Success:success Fail:fail]) {
        ...
        [self setGeneralPasteboard:[@"com.tencent.tencent" stringByAppendingString:[self keyFor:schema][@"appid"]] Value:authData encoding:TFYOSPboardEncodingKeyedArchiver];
 ...
 *  @return 需要打开的url
 */
+(NSString*)genShareUrl:(TFY_OSMessage*)msg to:(int)shareTo{
    ...
    if (msg.link&&!msg.multimediaType) {
        msg.multimediaType=TFYOSMultimediaTypeNews;
    }
    if ([msg isEmpty:@[@"image",@"link" ] AndNotEmpty:@[@"title"]]) {
        ...
        [self setGeneralPasteboard:@"com.tencent.mqq.api.apiLargeData" Value:data encoding: TFYOSPboardEncodingKeyedArchiver];
    ...
    }else  if ([msg isEmpty:@[] AndNotEmpty:@[@"title",@"desc",@"image",@"link",@"multimediaType"]]) {
...
+(BOOL)QQ_handleOpenURL{
    ...
    if ([url.scheme hasPrefix:@"QQ"]) {
        ...
        //登陆auth
        NSDictionary *ret=[self generalPasteboardData:[@"com.tencent.tencent" stringByAppendingString:[self keyFor:schema][@"appid"]] encoding:TFYOSPboardEncodingKeyedArchiver];
